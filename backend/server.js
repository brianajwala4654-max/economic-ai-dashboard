...
const countryInflation = {
  Kenya: {
    rate: 3.6,
    trend: [3.2, 3.4, 3.6, 3.8, 3.9, 3.7, 3.6, 3.5, 3.6, 3.7, 3.6, 3.6]
  },
  Uganda: {
    rate: 3.5,
    trend: [3.8, 3.6, 3.4, 3.2, 3.3, 3.5, 3.6, 3.5, 3.4, 3.5, 3.5, 3.5]
  },
  Tanzania: {
    rate: 3.1,
    trend: [3.5, 3.4, 3.3, 3.2, 3.1, 3.0, 3.1, 3.2, 3.1, 3.0, 3.1, 3.1]
  },
  Rwanda: {
    rate: 4.8,
    trend: [5.2, 5.0, 4.9, 4.8, 4.7, 4.8, 4.9, 4.8, 4.7, 4.8, 4.8, 4.8]
  },
  Ethiopia: {
    rate: 9.4,
    trend: [11.0, 10.5, 10.2, 9.9, 9.7, 9.5, 9.4, 9.4, 9.3, 9.4, 9.4, 9.4]
  }
};
...
app.get('/api/inflation/:country', (req, res) => {
  ...
  const observations = data.trend.map((value, i) => ({
    date: months[i],
...
Write in professional flowing paragraphs — no bullet points.`
    ...
    res.json({
      report: response.data.content[0].text
    });
  } catch (error) {
    console.error(
      'Claude API error:',
